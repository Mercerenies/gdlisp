...
pub struct StmtWalker {
  pub imp: Box<dyn Fn(&Stmt) -> Result<Vec<Stmt>, Error>>,
}
...
impl StmtWalker {
  ...
  pub fn new(function: impl Fn(&Stmt) -> Result<Vec<Stmt>, Error> + 'static) -> StmtWalker {
...
impl DeadCodeElimination {
  ...
  pub fn eliminate_on_stmts(&self, stmts: &[Stmt]) -> Result<Vec<Stmt>, Error> {
    let mut result = Vec::new();
    for stmt in stmts {
    ...
    match stmt {
      Stmt::IfStmt(stmt) => {
        if let Some(else_clause) = stmt.else_clause {
          if self.is_code_seq_noop(&else_clause) {
  ...
  pub fn is_code_seq_noop(&self, stmts: &[Stmt]) -> bool {
    stmts.iter().all(|stmt| self.is_code_noop(stmt))
  }

  pub fn is_code_noop(&self, stmt: &Stmt) -> bool {
    false ////
  }
...
impl FunctionOptimization for DeadCodeElimination {
  fn run(&self, function: &mut decl::FnDecl) -> Result<(), Error> {
    function.body = self.eliminate_on_stmts(&function.body)?;
    Ok(())
  }
}
*/
